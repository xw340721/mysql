...
function table($path){	
	...
	/**当前页截止的页数
     *主要是$pagetotal是ceil出来的最后一页会出现多出来的情况
     */
	$end=($presentpage==$pagetotal)? $total : ($presentpage*$num);  //结束记录

	/**下一页
     * 主要是控制最后一页的情况的时候是没有下一页
     * 通过设置的if的判断对echo输出进行控制
     */
    ...
    /**判断有没有$next
     * $next = ($presentpage = $pagetotal) 0 ? $presetnpage+1;
     */
	if($next){
	...
	echo "&nbsp;&nbsp;<a href='{$url}?page={$pagetotal}'>尾页</a>&nbsp;&nbsp;";
	}
	echo '</td></tr>';
	echo '</table>';
